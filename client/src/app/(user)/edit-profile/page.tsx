...
interface SocialMedia { linkedin: string; github: string; twitter: string; }
interface ErrorMessages { [key: string]: string; }
...
const EditProfile = () => {
  ...
  const [formState, setFormState] = useState({
    email: "", name: "", address: "", designation: "", contact: "",
    department: "", emergencyContact: "", employeeId: "", joiningDate: "",
    dob: "", mfaEnabled: false, profilePhoto: "", 
  ...
  useEffect(() => {
    if (user) {
      setFormState({
        ...
        designation: user.designation || "", contact: user.contact || "",
        department: user.department || "", emergencyContact: user.emergencyContact || "",
        employeeId: user.employeeId || "", joiningDate: user.joiningDate?.slice(0, 10) || "",
        dob: user.dob?.slice(0, 10) || "", profilePhoto: user.profilePhoto || "",
  ...
  const {
    name, email, dob, contact, emergencyContact, address, employeeId,
    department, designation, joiningDate, mfaEnabled, profilePhoto, socialMedia,
  } = formState;

  return (
    <div className="min-h-screen py-10 px-4 flex justify-center">
      <form onSubmit={handleUpdateProfile} className="w-full max-w-4xl border p-6 rounded-lg shadow-lg">
        ...
        <div className="flex justify-center mb-4">
          <label htmlFor="profilePhoto" className="cursor-pointer">
            <Image
              src={newProfilePhoto ? URL.createObjectURL(newProfilePhoto) : profilePhoto || "https://cdn-icons-png.flaticon.com/512/3135/3135715.png"}
              width={100} height={100} alt="Profile"
              className="w-24 h-24 object-cover rounded-full"
            />
            <input type="file" id="profilePhoto" accept="image/*" onChange={handleProfilePhotoChange} className="hidden" />
          </label>
        </div>
        ...
        <div className="mt-6 flex items-center space-x-4">
          ...
          <button type="button" onClick={() => handleFieldChange("mfaEnabled", true)} className={`px-3 py-1 rounded ${mfaEnabled ? "bg-blue-600 text-white" : "bg-gray-300"}`}>On</button>
          <button type="button" onClick={() => handleFieldChange("mfaEnabled", false)} className={`px-3 py-1 rounded ${!mfaEnabled ? "bg-blue-600 text-white" : "bg-gray-300"}`}>Off</button>
        </div>
